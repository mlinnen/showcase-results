...
| 3.2 | Name single-event lookup | 1. Navigate to `?name=Jane+Doe&year=2024` | Page displays ONLY 2024 results for Jane Doe; subtitle reads "Results for [Event Name] 2024"; no 2023 section appears | [ ] |
| 3.3 | ID single-event lookup | 1. Navigate to `?carver_id=16&year=2024` | Page displays results for carver #16 from 2024; page title shows carver's actual name (not the ID number) | [ ] |
...
| # | Test Case | Steps | Expected Result | Status |
|---|-----------|-------|-----------------|--------|
| 4.1 | ID without year (privacy constraint) | 1. Navigate to `?carver_id=16` (no year parameter) | Page displays error message: "A year is required when looking up by carver ID, because carver IDs differ between events. Try adding &year=2024 or search by name instead."; no PHP error or stack trace | [ ] |
| 4.2 | No parameters (usage guide) | 1. Navigate to component URL with no query string | Page displays usage instructions with examples: `?name=John+Doe`, `?name=John+Doe&year=2024`, `?carver_id=16&year=2024`; wrapped in `<div class="cca-usage">` | [ ] |
...
| 4.4 | ID not found (nonexistent ID) | 1. Navigate to `?carver_id=9999&year=2024` | Error message: "Carver #9999 was not found in the 2024 results."; no PHP warning about undefined array keys | [ ] |
| 4.5 | No results carver (registered, zero wins) | 1. Navigate to `?name=Bob+Wilson` (registered in competitors array but has no results)<br>2. Or use `?carver_id=8&year=2024` | Error message: "Bob Wilson is a registered competitor in 2024 but has no recorded results."; carver name displayed (not "Competitor #8") | [ ] |
...
| 4.7 | Invalid year (out of range) | 1. Navigate to `?name=John+Doe&year=1999` (year with no data file) | Error message: "No data available for 1999. Available years: 2024, 2023." (or similar, listing actually available years) | [ ] |
| 4.8 | Non-numeric ID (input validation) | 1. Navigate to `?carver_id=abc&year=2024` | Validation error: "Carver ID must be a number."; input rejected before database query | [ ] |
| 4.9 | Non-numeric year (input validation) | 1. Navigate to `?name=John+Doe&year=abcd` | Validation error: "Year must be a valid number."; input rejected before file access | [ ] |
...
| # | Test Case | Steps | Expected Result | Status |
|---|-----------|-------|-----------------|--------|
| 5.1 | Semantic HTML structure | 1. Navigate to `?name=John+Smith&year=2024`<br>2. View page source | Uses semantic HTML5 elements: `<section>`, `<table>`, `<thead>`, `<tbody>`, `<h1>`–`<h4>`; no `<div>` soup; tables have proper headers | [ ] |
...
| 6.2 | Multiple years coexist | 1. Ensure `results-2023.json` and `results-2024.json` both exist in media/data<br>2. Navigate to single-event URLs for each year | Both `?year=2023` and `?year=2024` work independently; files do not conflict or overwrite each other | [ ] |
...
1. **Potential year validation gap (ResultsService.php:182, 281)**
   - **Location**: `ResultsService::lookupByNameAndYear()` line 182, `lookupByCarverIdAndYear()` line 281
   - **Issue**: File path constructed as `$this->dataPath . '/results-' . $year . '.json'` without validating that `$year` is positive. If a negative year somehow passes validation (e.g., `-2024`), it creates path `results--2024.json` which is valid but unintended.
   - **Impact**: Low — HtmlView validates year is numeric and casts to int, but doesn't check for negative values. Unlikely in production but possible via direct service instantiation.
   - **Recommendation**: Add validation in `ResultsService::lookup()` to ensure `$year > 2000` (or similar reasonable lower bound) before dispatching to year-specific methods. Alternatively, validate in HtmlView that `$year >= 2000`.
...
- ✅ **carver_id requires year**: Enforced in HtmlView line 152-161 with clear error message (privacy-by-design requirement met)
